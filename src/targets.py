...
class TargetFieldGenerator:
    def __init__(self, grid_shape: Tuple[int, ...], max_distance: float = 5.0):
        ...
        if self.dims != 2:
            raise ValueError("TargetFieldGenerator only supports 2D grids.")
...
@nb.njit(cache=True)
def _compute_scalar_targets_2d_numba(
    scalar_map,
    best_score_map,
    starts,
    ends,
    ...
    num_segs = starts.shape[0]
    shape_x, shape_y = scalar_map.shape
    cutoff = max(2.5 * centerline_sigma, 1.5)
    sigma2 = max(centerline_sigma * centerline_sigma, 1e-8)
    normalizer = max(value_normalizer, 1e-8)
    ...
    for s in range(num_segs):
        ...
        scalar_value = min(1.0, max(0.0, value / normalizer))
        for i in range(min_x, max_x):
            for j in range(min_y, max_y):
                ap_x = i - start[0]
                ap_y = j - start[1]
                t = (ap_x * ab_x + ap_y * ab_y) / ab_norm2
                t = max(0.0, min(1.0, t))
                ...
                score = np.exp(-0.5 * d2 / sigma2)
                if score > best_score_map[i, j]:
                    best_score_map[i, j] = score
                    scalar_map[i, j] = scalar_value


class ScalarTargetGenerator2D:
    def __init__(
        self,
        grid_shape: Tuple[int, int],
        centerline_sigma: float = 0.85,
        value_normalizer: float = 1.0,
    ):
        if len(grid_shape) != 2:
            raise ValueError("ScalarTargetGenerator2D only supports 2D grids.")
        self.grid_shape = grid_shape
        self.centerline_sigma = float(centerline_sigma)
        self.value_normalizer = float(value_normalizer)

    def generate(self, segments: List[FiberSegment], values: np.ndarray) -> np.ndarray:
        scalar_map = np.zeros(self.grid_shape, dtype=np.float64)
        best_score_map = np.zeros(self.grid_shape, dtype=np.float64)

        if not segments:
            return scalar_map
        if len(segments) != len(values):
            raise ValueError("segments and values must have the same length.")

        starts = np.array([s.start for s in segments], dtype=np.float64)
        ends = np.array([s.end for s in segments], dtype=np.float64)
        values = np.asarray(values, dtype=np.float64)
        _compute_scalar_targets_2d_numba(
            scalar_map,
            best_score_map,
            starts,
            ends,
        ...
        return np.clip(scalar_map, 0.0, 1.0)
