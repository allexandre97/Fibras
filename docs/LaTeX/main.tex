...
\begin{itemize}
    ...
    \item \texttt{train.py}: train \texttt{STEDResUNet2D}.
    \item \texttt{test.py}: evaluate a saved checkpoint on the held-out split.
    \item \texttt{inference.py}: predict dense fields on a 2D image and export TIFF outputs.
    \item \texttt{visual.py} and \texttt{visualize\_dataset.py}: inspect datasets, predictions, and STED synthesis debug views.
\end{itemize}
...
start point, end point, and thickness multiplier. The project supports several
generators in \texttt{src/synthesis.py}:
\begin{itemize}
    \item \texttt{RandomWalkGenerator}: constrained short fibers with bounded turn angles.
    \item \texttt{SpaceColonizationGenerator}: branching tree-like structures.
    \item \texttt{CompositeGenerator}: mixtures of multiple generators.
\end{itemize}
...
\end{equation}
where $\hat{L}_V$ is a visibility logit. The older \texttt{FlexibleCVFUNet}
class remains in the codebase as a generic 2D/3D U-Net, but the upgraded training,
testing, and inference paths require \texttt{--dim 2} and use
\texttt{STEDResUNet2D}.
...
\section{Legacy and Supporting Modules}

Some modules still support older or auxiliary workflows. The volume-generation
branch can produce 3D samples with EDT and vector targets, and
\texttt{src/analysis.py} still contains direct density, topology, Hessian, and
structure-tensor analyzers. These are not the main upgraded STED training path.
They are useful as reference utilities, but current model training, evaluation,
and inference are explicitly 2D.
...
\end{verbatim}
\end{quote}
A saved checkpoint can be evaluated with \texttt{test.py} or applied to new
images with \texttt{inference.py}.
